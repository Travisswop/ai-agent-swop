...
import { RefreshCw, Menu, Loader2, TrendingUp, TrendingDown, Wallet, QrCode, Copy, Check } from 'lucide-react';
import Image from 'next/image';
import { toast } from '@/hooks/use-toast';
import { useEffect, useState } from 'react';
import { formatAddress, generateWalletFromPrivateKey } from '@/utils/wallet';
    ...
    <div className="w-[400px] h-screen bg-[#1a1a1a] border-l border-[#2a2a2a]">
      ...
      <div className="p-4 space-y-4 overflow-auto max-h-[calc(100vh-80px)]">
        ...
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-lg font-medium text-white">Token(s)</h3>
          <Button
            variant="ghost"
            size="sm"
                    ...
                    <div className="relative">
                      {token.logoURI ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-[#3a3a3a] flex items-center justify-center ring-2 ring-[#4a4a4a]">
                          <span className="text-sm font-medium text-gray-400">{token.symbol.slice(0, 2)}</span>
                        </div>
                      )}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {token.tags?.includes('native') && (
                          <span className="px-2 py-0.5 text-xs bg-blue-500/20 text-blue-400 rounded-full font-medium">Native</span>
                        )}
                        {token.tags?.includes('lp-token') && (
                          <span className="px-2 py-0.5 text-xs bg-purple-500/20 text-purple-400 rounded-full font-medium">LP</span>
                        )}
                      </div>
                  ...
                  <div className="text-right">
                    <div className="text-white font-medium text-lg group-hover:text-blue-400 transition-colors">
                      {token.amount.toLocaleString(undefined, { 
                        ...
                        maximumFractionDigits: token.tags.includes('native') ? 4 : 6 
                      })} {token.symbol}
                    </div>
                    <div className="flex items-center justify-end gap-2 mt-1">
                      <div className="text-sm font-medium text-gray-400">
                        ${(Number(token.price) * Number(token.amount)).toFixed(2)}
                      </div>
